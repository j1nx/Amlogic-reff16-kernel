...
static int ov3640_open(struct file *file)
{
	...
	if(OV3640_download_firmware(dev) >= 0) {
		while(((i2c_get_byte(client, 0x3f07) != 0x7f) || 
				(i2c_get_byte(client, 0x3f07) != 0x7e)) 
				&& i < 10) { //wait for the mcu ready 
        	...
        	const struct i2c_device_id *id)
{
	...
	wake_lock_init(&(t->wake_lock),WAKE_LOCK_SUSPEND, "ov3640");
	...
	if (plat_dat) {
		...
		if(t->platform_dev_data.device_init) {
			...
			printk("+++%s found a device_probe function," 
			" and run it..\n", __func__);
		}
		power_down_ov3640(t);
		if(t->platform_dev_data.device_uninit) {
			t->platform_dev_data.device_uninit();
			printk("+++%s found a uninit function,"
				" and run it..\n", __func__);
		}
	}
